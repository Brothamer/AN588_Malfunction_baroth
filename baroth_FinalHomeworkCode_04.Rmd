---
title: "BUlogin_FinalHomeworkCode_04"
...
An attempt at a version of the same function using t-distributions of Z scores instead of normal distributions to account for small sample size.
...
```
Yes it does, and it gives very similar results except the p value given by the t distribution method is higher and the confidence intervals are a little broader than those produced from the normal distribution method.
...
From the summary(model1) coefficients table above, the slope (β1) of longevity~brain size is 1.218 meaning that for every gram species mean brain size increases, the maximum longevity increases by 1.218 months. The p-value of the test that the slope differs from 0, <2e-16, is less than 0.05, indicates that the slope is not equal to 0.

```{r store slope and intercept}
model1_slope <- summary(model1)$coefficients["brainsize",]["Estimate"]
model1_intercept <- summary(model1)$coefficients["(Intercept)",]["Estimate"]
model1_slope_SE <- summary(model1)$coefficients["brainsize",]["Std. Error"]
```

```{r confidence interval}
lower <- model1_slope - qt(0.95, df = 211) * model1_slope_SE #213 observations - 2 = 211 df
upper <- model1_slope + qt(0.95, df = 211) * model1_slope_SE #1-.9 confidence - .1 divided between the lower and upper calculations
ci <- c(lower, upper)
ci
```
...
From the summary(model1) coefficients table above, the slope (β1) of log(longevity)~log(brain size) is 0.2341 meaning that for every unit increase in the logarithm of species mean brain size increases, the logarithm of maximum longevity increases by 1.218 units. The p-value of the test that the slope differs from 0, <2e-16, is less than 0.05, indicates that the slope is not equal to 0.

```{r store slope and intercept log}
model2_slope <- summary(model2)$coefficients["log_brainsize",]["Estimate"]
model2_intercept <- summary(model2)$coefficients["(Intercept)",]["Estimate"]
model2_slope_SE <- summary(model2)$coefficients["log_brainsize",]["Std. Error"]
```

```{r confidence interval log}
lower <- model2_slope - qt(0.95, df = 211) * model2_slope_SE
upper <- model2_slope + qt(0.95, df = 211) * model2_slope_SE
ci <- c(lower, upper)
ci
```
...
### longevity~brain size
```{r predict 800gm}
longevity800 <- model1_slope*(800) + model1_intercept # technically, this step isn't necessary as the predict() function also produces the point estimate, but this is the math that goes into that
longevity800 

pi800 <- predict(model1, newdata = data.frame(brainsize = 800), interval = "prediction", level = 0.90)
pi800 # fit is the point estimate and lwr and upr are the lower and upper prediction intervals
```
...
### log(longevity)~log(brain size)
```{r predict 800gm log}
log(800) # not necessary, just curious to know what the log of 800 is so I can visualize where this point is in the plot
log_longevity800 <- exp(model2_slope * log(800) + model2_intercept) # technically, this step isn't necessary as the predict() function also produces the point estimate, but this is the math that goes into that
log_longevity800

log_pi800 <- exp(predict(model2, newdata = data.frame(log_brainsize = log(800)), interval = "prediction", level = 0.90)) # fit is the point estimate and lwr and upr are the lower and upper prediction intervals
log_pi800
```
...
1. I noticed that Ritika has the date in her YAML set to the system date. I had forgotten that that was possible, and now I will set up my r markdowns like that.
2. I like that Ritika created object that contained her regression model formulas using the values as indexed from the model summary so that she could use that formula as a label on her plots. I just typed in my labels manually.

# What I learned
1. Based on Ritika's advice, I learned that it is useful to index the point estimates and other values from the model summaries that I will use in later analyses. That way, it is clear where the values come from and I do not have to go back and look to see what the value is every time I need to use it. 
2. I learned that my function should print the warning if n * po or n * (1-po) is less than or equal to 5 rather than just less than.
3. Through this whole assignment, I got a lot more familiar with writing functions and what I could have a function do. Specifically, I learned how to format if(){} else(){} commands and multi-part outputs.